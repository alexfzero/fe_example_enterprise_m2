  ...
  <div>
    ...
    <v-container class="d-flex flex-column justify-center align-center">
      ...
      >
        <template v-slot:[`item.actions`]="{ item }">
          ...
          <v-tooltip bottom>
            ...
            <span>Удалить тип операции</span>
          </v-tooltip>
        </template>
      </v-data-table>
      <TypeEditForm
      ...
      <ApplyDialog
          ...
          :get-text="() => `Вы уверены, что хотите удалить данный тип операции ?`"
...
import TypeEditForm from "@/modules/admin/operation_type/TypeEditForm.vue";
import ApplyDialog from "@/modules/admin/operation_type/ApplyDialog.vue";
...
export default {
  ...
  data() {
    return {
      ...
      count: 0,
      options: {},
      URL: urls.API + roles.SUPER_ADMIN + urls.TEMPS.NAME + urls.TEMPS.PATH.OPERATIONS,
      loadingPage: false,
      attrs: undefined,
      dialog_add_edit: false,
      dialog_delete: false,
      operation_data: undefined,
      edit_operation: undefined,
      items: undefined,
      headers: [
  ...
  methods: {
    ...
    newOperation() {
      ...
      this.edit_operation = false
      this.operation_data = undefined
    },
    editOperation(item) {
      this.dialog_add_edit = true
      this.edit_operation = true
      this.operation_data = item
    },
    ...
    closeDeleteDialog(event) {
      ...
      if (event) {
        let id_url = this.operation_data.id + '/'
