  ...
  <div>
    ...
    <v-container class="d-flex flex-column justify-center align-center">
      ...
      >
        <template v-slot:[`item.actions`]="{ item }">
          <v-tooltip bottom>
            ...
            <span>Редактировать статус</span>
          </v-tooltip>
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <v-icon
      ...
      <ApplyDialog
          ...
          :get-text="() => `Вы уверены, что хотите удалить данный статус ?`"
...
import StatusEditForm from "@/modules/admin/contract_status/StatusEditForm.vue";
...
export default {
  ...
  data() {
    return {
      ...
      count: 0,
      options: {},
      URL: urls.API + roles.SUPER_ADMIN + urls.TEMPS.NAME + urls.TEMPS.PATH.STATUSES,
      loadingPage: false,
      attrs: undefined,
  ...
  methods: {
    ...
    closeDeleteDialog(event) {
      ...
      if (event) {
        let id_url = this.status_data.id + '/'
