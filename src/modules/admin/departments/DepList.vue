  ...
  <div>
    ...
    <v-container class="d-flex flex-column justify-center align-center">
      ...
      >
        <template v-slot:[`item.actions`]="{ item }">
          <v-tooltip bottom>
            ...
            <span>Редактировать департамент</span>
          </v-tooltip>
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <v-icon
      ...
      <ApplyDialog
          ...
          :get-text="() => `Вы уверены, что хотите удалить данный департамент ?`"
...
import DepEditForm from "@/modules/admin/departments/DepEditForm.vue";
import ApplyDialog from "@/modules/admin/departments/ApplyDialog.vue";
...
export default {
  ...
  data() {
    return {
      ...
      count: 0,
      options: {},
      URL: urls.API + roles.SUPER_ADMIN + urls.TEMPS.NAME + urls.TEMPS.PATH.DEPARTMENTS,
      loadingPage: false,
      attrs: undefined,
      dialog_add_edit: false,
      dialog_delete: false,
      department_data: undefined,
      edit_department: undefined,
      items: undefined,
      headers: [
  ...
  methods: {
    ...
    newDepartment() {
      ...
      this.department_data = undefined
    },
    editDepartment(item) {
      this.dialog_add_edit = true
      this.edit_department = true
      this.department_data = item
    },
    ...
    closeDeleteDialog(event) {
      ...
      if (event) {
        let id_url = this.department_data.id + '/'
